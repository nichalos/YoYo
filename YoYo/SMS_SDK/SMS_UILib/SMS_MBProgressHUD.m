...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        if (nil != self.detailsLabelText) {
            ...
            NSAttributedString *attrStr = [[NSAttributedString alloc] initWithString:detailsLabel.text];
            detailsLabel.attributedText = attrStr;
            NSRange range = NSMakeRange(0, attrStr.length);
            NSDictionary *dic = [attrStr attributesAtIndex:0 effectiveRange:&range];   // 获取该段attributedString的属性字典
            // 计算文本的大小
            CGSize labelSize = [detailsLabel.text boundingRectWithSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) // 用于计算文本绘制时占据的矩形块
