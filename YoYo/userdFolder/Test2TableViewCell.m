...
//        [self.timeLabel sizeToFit];
        ...
        [self.bodyLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeTop ofView:self.timeLabel withOffset:0.f];
        [self.bodyLabel autoPinEdge:ALEdgeLeft toEdge:ALEdgeRight ofView:self.timeLabel];
        [self.bodyLabel autoPinEdgeToSuperviewEdge:ALEdgeTrailing withInset:10.f];
        [self.bodyLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeTop ofView:self.timeLabel];
        [self.bodyLabel autoPinEdgeToSuperviewEdge:ALEdgeBottom withInset:10 relation:NSLayoutRelationGreaterThanOrEqual];

//        [self.creatTimeLabel autoPinEdge:ALEdgeLeft toEdge:ALEdgeLeft ofView:self.bodyLabel];
//        [self.creatTimeLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:self.bodyLabel withOffset:5.0f];
//        [self.creatTimeLabel autoPinEdgeToSuperviewEdge:ALEdgeBottom withInset:10 relation:NSLayoutRelationGreaterThanOrEqual];
...
-(void)setImageswithURLs:(NSArray *)urls
{
    ...
    for (NSString* url in _urlArray)
    {
      ...
      //  [self.contentView addSubview:imgV];
        ...
        NSLog(@"count = 0-------------%d",self.imageViewArray.count);
    }
...
-(void)layoutImagesInContentView
{
    ...
    if (imageCount==1)
    {
        ...
        [imgView autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:self.bodyLabel withOffset:5.0f];
        
//        [imgView autoPinEdgeToSuperviewEdge:ALEdgeLeading withInset:12.f];
        [imgView autoPinEdge:ALEdgeLeft toEdge:ALEdgeLeft ofView:self.bodyLabel];
...
//        [prototypeImage autoPinEdgeToSuperviewEdge:ALEdgeLeading withInset:12.f];
        [prototypeImage autoPinEdge:ALEdgeLeft toEdge:ALEdgeLeft ofView:self.bodyLabel];
   ...
   NSLayoutConstraint*  firtImageLayout=
        ...
        //判断有几行
        NSInteger row=_imageViewArray.count%3==0?_imageViewArray.count/3:_imageViewArray.count/3+1;
        NSInteger count=0;
...
-(UILabel *)timeLabel
{
    if (!_timeLabel)
    {
        ...
        _timeLabel.font=[UIFont fontWithName:@"Helvetica Neue" size:12];
    }
    return _timeLabel;
}
-(UILabel *)creatTimeLabel{
    if (!_creatTimeLabel)
    {
        _creatTimeLabel=[[UILabel alloc]initForAutoLayout];
        _creatTimeLabel.textColor=[UIColor grayColor];
        _creatTimeLabel.font=[UIFont fontWithName:@"Helvetica Neue" size:8];
    }
    return _timeLabel;
}
